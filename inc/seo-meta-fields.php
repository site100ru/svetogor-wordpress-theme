        ...
        jQuery(document).ready(function($) {
            ...
            function updateCounter(input, counter, status) {
                ...
                if (input.attr('id') === 'seo_title') {
                    if (length === 0) {
                        statusText = 'Будет использован заголовок страницы';
                        statusColor = '#999';
                    } else if (length < 40) {
                        statusText = '⚠️ Слишком короткий';
                        statusColor = '#d63638';
                    } else if (length <= 60) {
                        statusText = '✓ Оптимально';
                        statusColor = '#00a32a';
                    } else if (length <= 70) {
                        statusText = '⚠️ Немного длинный';
                        statusColor = '#dba617';
                    } else {
                        statusText = '❌ Слишком длинный';
                        statusColor = '#d63638';
                    }
                } else {
                    if (length === 0) {
                        statusText = 'Описание не задано';
                        statusColor = '#999';
                    } else if (length < 100) {
                        statusText = '⚠️ Слишком короткое';
                        statusColor = '#d63638';
                    } else if (length <= 160) {
                        statusText = '✓ Оптимально';
                        statusColor = '#00a32a';
                    } else if (length <= 200) {
                        statusText = '⚠️ Немного длинное';
                        statusColor = '#dba617';
                    } else {
                        statusText = '❌ Слишком длинное';
                        statusColor = '#d63638';
                    }
                }
            ...
            const titleInput = $('#seo_title'),
                titleCounter = $('#seo_title_counter'),
                titleStatus = $('#seo_title_status');
            const descInput = $('#seo_description'),
                descCounter = $('#seo_description_counter'),
                descStatus = $('#seo_description_status');
...
function render_taxonomy_seo_fields_edit($term)
{
    ...
    if ($term->taxonomy === 'product_cat') {
        ...
        if ($thumbnail_id) {
            ...
            $default_image_label = 'Миниатюра комплексного оформления';
        }
    }
...
?>
    ...
    <tr class="form-field">
        ...
        <td style="padding: 15px 10px;">
            ...
            <p class="description">
                ...
                <br><strong>Текущая длина: <span id="term_seo_title_length_<?php echo esc_attr($form_id); ?>"><?php echo mb_strlen($seo_title); ?></span> символов</strong>
            </p>
        </td>
    </tr>
    ...
    <tr class="form-field">
        ...
        <td style="padding: 15px 10px;">
            ...
            <p class="description">
                ...
                <br><strong>Текущая длина: <span id="term_seo_desc_length_<?php echo esc_attr($form_id); ?>"><?php echo mb_strlen($seo_description); ?></span> символов</strong>
            </p>
        </td>
    </tr>
    ...
    <tr class="form-field">
        ...
        <td style="padding: 15px 10px;">
            ...
            <input type="hidden" id="term_default_image_<?php echo esc_attr($form_id); ?>" value="<?php echo esc_attr($default_image_url); ?>" />

            <div style="display: flex; align-items: flex-start; gap: 15px;">
                <div id="term_seo_image_preview_<?php echo esc_attr($form_id); ?>" style="border: 1px solid #ddd; border-radius: 4px; overflow: hidden; width: 150px; height: 150px; display: flex; align-items: center; justify-content: center; background: #f9f9f9; position: relative;">
                    <?php if ($seo_image):
                        $image_url = wp_get_attachment_url($seo_image);
                    ?>
                        <img loading="lazy" src="<?php echo esc_url($image_url); ?>" style="width: 100%; height: 100%; object-fit: cover;" alt="SEO изображение">
                    <?php elseif ($default_image_url): ?>
                        <img loading="lazy" src="<?php echo esc_url($default_image_url); ?>" style="width: 100%; height: 100%; object-fit: cover; opacity: 0.6;" alt="Изображение по умолчанию">
                        <div style="position: absolute; bottom: 5px; left: 5px; font-size: 10px; color: #666; background: rgba(255,255,255,0.9); padding: 2px 5px; border-radius: 3px;">по умолчанию</div>
                    <?php else: ?>
                        <span style="color: #999; font-size: 12px;">Нет изображения</span>
                    <?php endif; ?>
                </div>
                ...
                <div style="flex: 1;">
                    ...
                    <p class="description" style="margin: 0;">
                        <?php if ($default_image_url): ?>
                            <strong style="color: #0073aa;">✓ По умолчанию используется:</strong><br>
                            <?php echo esc_html($default_image_label); ?><br>
                        <?php else: ?>
                            Если не выбрать, будет использован логотип сайта<br>
                        <?php endif; ?>
                        <span style="color: #666;">Рекомендуемый размер: 1200×630 px</span>
                    </p>
                </div>
            </div>
        </td>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            var mediaUploader;
            var formId = '<?php echo esc_js($form_id); ?>';
            var defaultImageUrl = $('#term_default_image_' + formId).val();
            ...
            $('#term_remove_seo_image_button_' + formId).click(function(e) {
                ...
                if (defaultImageUrl) {
                    $('#term_seo_image_preview_' + formId).html(
                        '<img loading="lazy" src="' + defaultImageUrl + '" style="width: 100%; height: 100%; object-fit: cover; opacity: 0.6;">' +
                        '<div style="position: absolute; bottom: 5px; left: 5px; font-size: 10px; color: #666; background: rgba(255,255,255,0.9); padding: 2px 5px; border-radius: 3px;">по умолчанию</div>'
                    );
                } else {
                    $('#term_seo_image_preview_' + formId).html('<span style="color: #999; font-size: 12px;">Нет изображения</span>');
...
function output_seo_meta_tags()
{
    ...
    elseif (is_category() || is_tag() || is_tax()) {
        ...
        if ($seo_image_id) {
            ...
            if ($term->taxonomy === 'product_cat') {
                $category_photo_id = get_term_meta($term->term_id, 'category_photo', true);
                if ($category_photo_id) {
                    $seo_image = wp_get_attachment_url($category_photo_id);
                }
            } elseif ($term->taxonomy === 'complex_design') {
                $thumbnail_id = get_term_meta($term->term_id, 'thumbnail_id', true);
                if ($thumbnail_id) {
    ...
    // Для архивных страниц других типов записей
    ...
    if (is_singular()) {
        ...
        if ($posts_page_id) {
            $current_url = get_permalink($posts_page_id);
        } else {
