...
if ($_POST) {
    ...
    error_log('reCAPTCHA response (calculation): ' . print_r($Return, true));
    ...
    if ($Return->success == true && $Return->score > 0.125) {
        ...
        if ($name !== 'Не указано' && $phone !== 'Не указан' && $email !== 'Не указан') {
            ...
            if ($emailSent) {
                $_SESSION['win'] = 1;
                $_SESSION['recaptcha'] = '<p class="text-success">Спасибо за заявку! Мы подготовим для Вас индивидуальное предложение и свяжемся в ближайшее время.</p>';
            } else {
                $_SESSION['win'] = 1;
